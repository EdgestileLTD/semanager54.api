...
class Product extends Base
{
    ...
    {
        return [
            "select" => 'sp.*, so.article article,
                sop.value price, sb.name name_brand, sg.name name_group,
            ...
            "joins" => [
                ...
                [
                    "type" => "left",
                    "table" => 'shop_brand sb',
                    "condition" => "sp.id_brand = sb.id"
                ],
                [
                    "type" => "left",
                    "table" => 'shop_offer_stock sos',
                    "condition" => "sos.id_offer = so.id"
                ],
                [
                    "type" => "left",
    ...
    {
        return [
            "select" => 'sp.*, sb.name name_brand, sm.name name_measure',
            "joins" => [
                [
                    "type" => "left",
                    "table" => 'shop_brand sb',
                ...
                [
                    "type" => "left",
                    "table" => 'shop_measure sm',
                    "condition" => 'sm.id = sp.id_measure'
                ],
            ]
        ];
    }
    ...
    public function getImages($idProduct = null)
    {
        ...
        $u->select('si.id, si.id_image, img.name,  
                    CONCAT(f.name, "/", img.name) image_path, si.is_main, img.alt, si.sort');
        $u->innerJoin("image img", "img.id = si.id_image");
        $u->leftJoin("image_folder f", "f.id = img.id_folder");
        $u->where('si.id_product = ?', $id);
        $u->orderBy("si.sort");

        return parent::correctItemsBeforeFetch($u->getList());
    }
...
//                $this->input["idGroup"] = Category::getIdGroupByColumns($this->input);
    ...
    {
        $items = parent::correctItemsBeforeFetch($items);
        foreach ($items as &$item) {
